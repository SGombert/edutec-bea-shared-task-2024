...
for i, (idx_train, idx_test) in enumerate(rkfold.split(dataset)):
    X_tr = dataset_X(dataset.iloc[idx_train], '[SEP]')
    Y_tr = dataset_y(dataset.iloc[idx_train])
    X_ev = dataset_X(dataset.iloc[idx_test], '[SEP]')
    Y_ev = dataset_y(dataset.iloc[idx_test])

    model = ScoringModel('bert-large-uncased', f'run{i}').to(device)
    model.set_dataset(X_tr, torch.tensor(Y_tr, dtype=torch.float).to(device))
    model.set_evalset(X_ev, torch.tensor(Y_ev, dtype=torch.float).to(device))
    ...
    model.fit(4, optimizer, scheduler=scheduler, batch_size=1)

    evals.append(model.self_eval())
...
with open('5x5-bert.txt', 'w', encoding='utf8') as f:
