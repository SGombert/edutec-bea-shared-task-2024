...
x_data = dataset_X(dataset, '[SEP]')

model = ScoringModel('google/electra-large-discriminator', 'pfx').to(device)
model.eval()

model.load_state_dict(torch.load('final-model-electra.pt'))
model.eval()

ret = {
    'ItemNum': dataset['ItemNum'].values,
    'Prediction': [
        model.forward(x)[0][0].cpu().detach().numpy() for x in tqdm(x_data)
    ]
}

pd.DataFrame.from_dict(ret).to_csv('EduTec_Difficulty_Predictions_electra.csv')

ret = {
    'ItemNum': dataset['ItemNum'].values,
    'Prediction': [
        model.forward(x)[0][1].cpu().detach().numpy() * 100 for x in tqdm(x_data)
    ]
}

pd.DataFrame.from_dict(ret).to_csv('EduTec_Response_time_predictions_electra.csv')
