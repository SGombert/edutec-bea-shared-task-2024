...
for i, (idx_train, idx_test) in enumerate(rkfold.split(dataset)):
    vectorizer_main = TfidfVectorizer(analyzer='char_wb', ngram_range=(3, 3))
    vectorizers_answers = dict(
        [
            (nn, TfidfVectorizer(analyzer='char_wb', ngram_range=(3, 3)))
            for nn
    ...
    model_diff = DummyRegressor()
    model_time = DummyRegressor()
    ...
    res = {
        'overall': (pearsonr(diff_e, diff_r).statistic + pearsonr(time_e, time_r).statistic + spearmanr(diff_e, diff_r).statistic + spearmanr(time_e, time_r).statistic) / 4, 
...
with open('5x5-dummy.txt', 'w', encoding='utf8') as f:
